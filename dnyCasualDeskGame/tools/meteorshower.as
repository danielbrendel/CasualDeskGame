...
class CDamageDecal : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(vec[0] + 35, vec[1] + 10);
		this.m_hSprite = R_LoadSprite(g_szToolPath + "decal.png", 1, 64, 64, 1, false);
		this.m_oLifeTime.SetDelay(20);
		this.m_oLifeTime.Reset();
		this.m_oLifeTime.SetActive(true);
		this.m_oModel.Alloc();
		BoundingBox bbox;
...
const int SPRITESHEET_INDEX_START = 11;
const int SPRITESHEET_INDEX_END = 28;
class CFireball : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecDest;
	Vector m_vecSelSize;
	...
	void Move()
	{
		...
		this.m_vecPos[0] += int(sin(this.m_fRotation + 1.10) * this.m_iSpeed);
		this.m_vecPos[1] -= int(cos(this.m_fRotation + 1.10) * this.m_iSpeed);
	}
	...
	{
		this.m_vecDest = vec;
		this.m_fRotation = atan2(float(this.m_vecDest[1] - this.m_vecPos[1]), float(this.m_vecDest[0] - this.m_vecPos[0])) + (Util_Random(250, 650) / 1000.0);
		this.m_iAliveTime = (this.m_vecPos.Distance(this.m_vecDest) + (Util_Random(0, 50) - 50)) * 3 + 550;
		this.m_tmrMovement.SetDelay(this.m_iSpeed);
		this.m_tmrMovement.Reset();
		this.m_tmrMovement.SetActive(true);
		this.m_tmrAlive.SetDelay(this.m_iAliveTime);
		this.m_tmrAlive.Reset();
		this.m_tmrAlive.SetActive(true);
		this.m_hSprite = R_LoadSprite(g_szToolPath + "fireball.png", 40, 128, 96, 4, false);
		SoundHandle hSpawn = S_QuerySound(g_szToolPath + "spawn.wav");
		S_PlaySound(hSpawn, 10);
		this.m_oModel.Alloc();
	}
...
void CDG_API_Process()
{
	if (g_bRunning) {
		g_tmrVolley.Update();
		if (g_tmrVolley.IsElapsed()) {
			g_tmrVolley.Reset();
			CFireball @fb = CFireball();
			Ent_SpawnEntity(@fb, g_vecMousePos);
		}

		g_tmrShower.Update();
		if (g_tmrShower.IsElapsed()) {
			g_tmrShower.SetActive(false);
			g_bRunning = false;
		}
	}
}
...
{
	if (!g_bRunning) {
		g_tmrShower.SetDelay(15000);
		g_tmrShower.Reset();
		g_tmrShower.SetActive(true);
		
		g_tmrVolley.SetDelay(210);
		g_tmrVolley.Reset();
		g_tmrVolley.SetActive(true);

		g_bRunning = true;

		g_vecMousePos = vAtPos;
	}
}
