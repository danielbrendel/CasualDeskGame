/*
	...
	Tool: Flamethrower gun (developed by Daniel Brendel)
	Version: 0.2
...
class CFlame : IScriptedEntity
{
	...
	void Move(void)
	{
		this.m_vecPos[0] -= int(this.m_fSpeed);
		this.m_vecPos[1] -= int(this.m_fSpeed);
	}
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_oMoveSpeed.SetDelay(10);
		this.m_oMoveSpeed.Reset();
		this.m_oMoveSpeed.SetActive(true);
	...
	void OnProcess()
	{
		...
		this.m_oMoveSpeed.Update();
		if (this.m_oMoveSpeed.IsElapsed()) {
			if (this.m_fSpeed != 0.0)
				this.m_fSpeed -= 1;
		}
		this.Move();
	}
...
{
	CFlame@ obj = CFlame();
	Ent_SpawnEntity(@obj, Vector(vAtPos[0] - 8, vAtPos[1] + 10));
