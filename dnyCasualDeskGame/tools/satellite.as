/*
	...
	Tool: Satellite (developed by Daniel Brendel)
	Version: 0.1
...
class CExplosion : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(vec[0] + 15, vec[1] - 28);
...
class CDamageDecal : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(vec[0] + 5, vec[1] - 50);
		this.m_hSprite = R_LoadSprite(g_szToolPath + "decal.png", 1, 64, 64, 1, false);
...
class CLightning : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecTarget;
	Model m_oModel;
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		SoundHandle hAttack = S_QuerySound(g_szToolPath + "attack.wav");
		S_PlaySound(hAttack, 10);
		this.m_oModel.Alloc();
	}
	...
	void OnRelease()
	{
		...
		Ent_SpawnEntity(@expl, this.m_vecTarget);

		CDamageDecal @dcl = CDamageDecal();
		Ent_SpawnEntity(@dcl, this.m_vecTarget);
	}
...
const int SATELLITE_ATTACK_DISTANCE = 250;
const int SATELLITE_SPEED = 5;
class CSatellite : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecTarget;
	Vector m_vecSelSize;
	...
	bool m_bMoveBack;
	bool m_bRemove;
	SpriteHandle m_hSatellite;
	
	CSatellite()
    {
		this.m_vecSelSize = Vector(0, 0);
		this.m_bMoveBack = false;
	...
	void Move()
	{
		...
		if (!this.m_bMoveBack) {
			this.m_vecPos[1] += SATELLITE_SPEED;

			if (this.m_vecPos.Distance(this.m_vecTarget) < SATELLITE_ATTACK_DISTANCE) {
				this.m_tmrMovement.SetActive(false);
			}
		} else {
			this.m_vecPos[1] -= SATELLITE_SPEED;

			if (this.m_vecPos[1] < -100) {
				this.m_bRemove = true;
			}
		}
	}
	...
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(vec[0], -100);
		...
		this.m_hSatellite = R_LoadSprite(g_szToolPath + "satellite.png", 1, 69, 115, 1, false);
		SoundHandle hCharge = S_QuerySound(g_szToolPath + "charge.wav");
		S_PlaySound(hCharge, 1);
		this.m_oModel.Alloc();
	}
	...
	void OnProcess()
	{
		if (this.m_tmrMovement.IsActive()) {
			...
			if (!this.m_tmrAttack.IsActive()) {
				...
				CLightning @obj = CLightning();
				obj.SetTarget(Vector(this.m_vecTarget[0], this.m_vecTarget[1] + 50));
				obj.SetRotation(0.0f);
				Ent_SpawnEntity(@obj, Vector(this.m_vecPos[0] + 18, this.m_vecPos[1] + 110));
			}
		}
	...
	void OnDrawOnTop()
	{
		R_DrawSprite(this.m_hSatellite, this.m_vecPos, 0, 0.0f, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
	}
