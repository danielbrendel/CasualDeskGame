...
class CDecalSprite : IScriptedEntity
{
	...
	void OnDraw()
	{
		R_DrawSprite(this.m_hSprite, this.m_vecPos, 0, 0.0, Vector(-1, -1), 2.0, 2.0, false, Color(0, 0, 0, 0));
	}
...
class CDecalEntity : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(0, 0), Vector(18, 18));
		this.m_oModel.Alloc();
...
class CLaserEntity : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hSprite = R_LoadSprite(g_szToolPath + "laser" + (Util_Random(0, 4) + 1) + ".png", 1, 60, 99, 1, false);
		this.m_tmrLifeTime.SetDelay(700 + Util_Random(0, 100));
