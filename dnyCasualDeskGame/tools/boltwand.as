/*
	...
	Tool: Bolt Wand (developed by Daniel Brendel)
	Version: 0.1
...
class CExplosion : IScriptedEntity
{
	...
	{
		CDecal @dcl = CDecal();
		Ent_SpawnEntity(@dcl, Vector(this.m_vecPos[0] - 20, this.m_vecPos[1] - 20));
	}
...
class CLightning : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		SoundHandle hAttackSound = S_QuerySound(g_szToolPath + "attack.wav");
		S_PlaySound(hAttackSound, 10);
		this.m_oModel.Alloc();
	}
	...
	void OnRelease()
	{
		Vector vecSpawnPos = Vector(g_vecCursorPos[0] - 200, g_vecCursorPos[1] - 195);
		CExplosion @expl = CExplosion();
		Ent_SpawnEntity(@expl, vecSpawnPos);
	}
...
void CDG_API_DrawOnTop()
{
	if (g_bSelectionStatus) {
		Vector vecDrawPos = Vector(g_vecCursorPos[0] - 180, g_vecCursorPos[1] - 175);
...
{
	Vector vecTargetPos = Vector(g_vecCursorPos[0] - 200, g_vecCursorPos[1] - 195);
	CLightning @obj = CLightning();
	obj.SetRotation(2.2);
	obj.SetTarget(vecTargetPos);
	Ent_SpawnEntity(@obj, Vector(vAtPos[0] + 110, vAtPos[1] + 50));
}
