...
namespace Workshop {
	...
	public:
		...
		bool InitWorkshopUpdate(const std::wstring& wszTool)
		{
			//Initialize workshop update of tool

			this->m_bSuccess = false;
			this->m_bJobDone = false;
			this->m_wszTargetTool = this->m_wszPath + L"tools\\" + wszTool;

			if (Utils::FileExists(this->m_wszPath + L"tools\\" + wszTool + L"\\workshop_id.txt")) { //Check if Workshop item shall be updated
				if (!this->StartItemUpdate(this->GetWorkshopId(this->m_wszPath + L"tools\\" + wszTool + L"\\workshop_id.txt"))) { //Start item update
		...
		bool StartItemUpdate(PublishedFileId_t workshopId)
		{
			...
			if (hUpdate) {
				...
				if (!LoadWorkshopInfoData(this->m_wszTargetTool, sData)) //Load Workshop data
					return false;
