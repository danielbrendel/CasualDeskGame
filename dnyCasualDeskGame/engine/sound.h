...
namespace DxSound {
	...
	public:
		...
		bool LoadGlobalVolume(const std::wstring& wszFile)
		{
			//Load global volume
			...
			if (hFile.is_open()) {
				wchar_t wszLine[MAX_PATH];
				hFile.getline(wszLine, sizeof(wszLine));
				this->SetGlobalVolume(_wtoi(wszLine));
		...
		bool StoreGlobalVolume(const std::wstring& wszFile)
		{
			//Store global volume

			if (Utils::FileExists(wszFile))
				DeleteFile(wszFile.c_str());

			std::wofstream hFile;
			hFile.open(wszFile, std::wofstream::out);
			if (hFile.is_open()) {
				hFile << std::to_wstring(this->GetGlobalVolume()) << std::endl;
				hFile.close();
