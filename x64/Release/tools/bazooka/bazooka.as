...
class CRocketEntity : IScriptedEntity
{
    ...
    {
		this.m_fSpeed = 10;
		this.m_fWalkRot = 200.0;
		this.m_iCurrentTrail = 0;
		this.m_bGotDamage = false;
    }
	...
	{
		this.m_vecPos = vec;
		this.m_hRocket = R_LoadSprite(g_szToolPath + "rocket.png", 1, 80, 60, 1, false);
		this.m_hTrail = R_LoadSprite(g_szToolPath + "trail.png", 8, 128, 128, 8, false);
		this.m_tmrAliveTime.SetDelay(1100);
		this.m_tmrAliveTime.Reset();
		this.m_tmrAliveTime.SetActive(true);
		this.m_tmrTrail.SetDelay(1);
		this.m_tmrTrail.Reset();
		this.m_tmrTrail.SetActive(true);
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(0, 0), Vector(80, 60));
		this.m_oModel.Alloc();
		this.m_oModel.Initialize2(bbox, this.m_hRocket);
	}
	...
	void OnRelease()
	{
		...
		Ent_SpawnEntity(@dcl, Vector(this.m_vecPos[0] + 20, this.m_vecPos[1]));
	...
	void OnProcess()
	{
		...
		this.m_tmrAliveTime.Update();
		
		this.m_tmrTrail.Update();
		if (this.m_tmrTrail.IsElapsed()) {
			this.m_tmrTrail.Reset();
			this.m_iCurrentTrail++;
			if (this.m_iCurrentTrail >= 64)
				this.m_iCurrentTrail = 0;
		}
	}
	...
	void OnDrawOnTop()
	{
		R_DrawSprite(this.m_hRocket, this.m_vecPos, 0, this.m_fWalkRot, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
		R_DrawSprite(this.m_hTrail, Vector(this.m_vecPos[0] + 55, this.m_vecPos[1] + 10), this.m_iCurrentTrail, 2.1, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
	}
	...
	bool NeedsRemoval()
	{
		return this.m_tmrAliveTime.IsElapsed() || this.m_bGotDamage;
	}
...
{
	g_hCrossHair = R_LoadSprite(g_szToolPath + "xhair.png", 1, 32, 32, 1, false);
...
{
	R_DrawSprite(g_hCrossHair, Vector(g_vMousePos[0] - 300, g_vMousePos[1] - 200), 0, 0.0, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
}
...
void CDG_API_Trigger(const Vector& in vAtPos)
{
	...
	Ent_SpawnEntity(@obj, Vector(vAtPos[0] - 20, vAtPos[1] - 50));
	SoundHandle hShoot = S_QuerySound(g_szToolPath + "shoot.wav");
	S_PlaySound(hShoot, 10);
}
...
bool CDG_API_QueryToolInfo(HostVersion hvVersion, ToolInfo &out info, const GameKeys& in gamekeys, const string &in szToolPath)
{
	...
	info.iCursorWidth = 128;
	info.iCursorHeight = 32;
	info.uiTriggerDelay = 1000;
