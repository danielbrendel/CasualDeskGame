/*
	...
	Tool: Rocket launcher vehicle (developed by Daniel Brendel)
	Version: 0.1
...
class CBaseRLauncher : IScriptedEntity
{
	Vector m_vecPosition;
	Model m_oModel;
	SpriteHandle m_hSprBody;
	float m_flBodyRot;
	RLauncherAttributes m_sRLauncherAttrs;
	Timer m_tmrAiming;
	float m_flAimDest;
	float m_flAimStep;
	Timer m_tmrMovement;
	...
	//Getters
	int GetHealth() { return this.m_sRLauncherAttrs.Health; }
	int GetDamage() { return this.m_sRLauncherAttrs.Damage; }
	int GetSpeed() { return this.m_sRLauncherAttrs.Speed; }
	float GetRotation_() { return this.m_sRLauncherAttrs.Rotation; }
	uint8 GetTeam() { return this.m_sRLauncherAttrs.Team; }
	
	//Setters
	void SetHealth(int iHealth) { this.m_sRLauncherAttrs.Health = iHealth; }
	void SetDamage(int iDamage) { this.m_sRLauncherAttrs.Damage = iDamage; }
	void SetSpeed(int iSpeed) { this.m_sRLauncherAttrs.Speed = iSpeed; }
	void SetRotation_(float fRotation) { this.m_sRLauncherAttrs.Rotation = fRotation; }
	void SetTeam(uint8 ui8Team) { this.m_sRLauncherAttrs.Team = ui8Team; }
	void SetBodySize(const Vector& in vec) { this.m_sRLauncherAttrs.BodySize = vec; }
	...
	void Move()
	{
		...
		//Set next position according to view
		this.m_vecPosition[0] += int(sin(this.m_flBodyRot + 0.014) * this.m_sRLauncherAttrs.Speed);
		this.m_vecPosition[1] -= int(cos(this.m_flBodyRot + 0.014) * this.m_sRLauncherAttrs.Speed);
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hSprBody = R_LoadSprite(g_szToolPath + "vehicle.png", 1, this.m_sRLauncherAttrs.BodySize[0], this.m_sRLauncherAttrs.BodySize[1], 1, false);
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(0, 0), Vector(this.m_sRLauncherAttrs.BodySize[0], this.m_sRLauncherAttrs.BodySize[1]));
		this.m_oModel.Alloc();
		this.m_oModel.SetCenter(Vector(this.m_sRLauncherAttrs.BodySize[0] / 4, this.m_sRLauncherAttrs.BodySize[1] / 4));
	...
	void OnRelease()
	{
		...
		CExplosion@ expl = CExplosion();
		Ent_SpawnEntity(@expl, Vector(this.m_vecPosition[0] - this.m_sRLauncherAttrs.BodySize[0], this.m_vecPosition[1] - this.m_sRLauncherAttrs.BodySize[1]));
	...
	void OnProcess()
	{
		...
		if (this.m_tmrDirMove.IsActive()) {
			...
			this.m_flBodyRot += this.m_sRLauncherAttrs.Rotation;
	...
	bool NeedsRemoval()
	{
		return this.m_sRLauncherAttrs.Health <= 0;
	}
	...
	void OnDamage(DamageValue dv)
	{
		this.m_sRLauncherAttrs.Health -= dv;
	}
	...
	DamageValue GetDamageValue()
	{
		return this.m_sRLauncherAttrs.Damage;
	}
	
	//Return a name string here, e.g. the class name or instance name. This is used when DAMAGE_NOTSQUAD is defined as damage-type, but can also be useful to other entities
	string GetName()
	{
		return "CBaseRLauncher";
	}
...
class CRLauncher : CBaseRLauncher
{
	CRLauncher()
	{
		this.SetHealth(1000);
...
void CDG_API_DrawOnTop()
{
	if (g_bSelectionStatus) {
		string szInfoStr = (g_bTeamSelection) ? "[RLauncher] Team 1" : "[RLauncher] Team 2";
...
{
	info.szName = "Rocket Launcher Vehicle";
