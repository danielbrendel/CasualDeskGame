...
class CBigDecalSprite : IScriptedEntity
{
	Vector m_vecPos;
	Model m_oModel;
	Timer m_oLifeTime;
	...
	CBigDecalSprite()
    {
		this.m_fRotation = Util_Random(1, 630) / 100.0;
    }
...
class CExplosion : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(vec[0] - 220, vec[1] - 220);
		this.m_hSprite = R_LoadSprite(g_szToolPath + "explosion.png", 64, 512, 512, 8, false);
		this.m_oExplosion.SetDelay(40);
		this.m_oExplosion.Reset();
		this.m_oExplosion.SetActive(true);
		this.m_oFlash.SetDelay(500);
		this.m_oFlash.Reset();
		this.m_oFlash.SetActive(true);
		this.m_hSound = S_QuerySound(g_szToolPath + "explosion.wav");
		S_PlaySound(this.m_hSound, 10);
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(0, 0), Vector(512, 512));
		this.m_oModel.Alloc();
		this.m_oModel.Initialize2(bbox, this.m_hSprite);
	...
	void OnProcess()
	{
		if (this.m_oFlash.IsActive()) {
			this.m_oFlash.Update();
			if (this.m_oFlash.IsElapsed()) {
				this.m_oFlash.SetActive(false);
			}
		}
	...
	void OnDrawOnTop()
	{
		R_DrawSprite(this.m_hSprite, this.m_vecPos, this.m_iFrameCount, this.m_fRotation, Vector(-1, -1), 2.5, 2.5, false, Color(0, 0, 0, 0));
		
		if (this.m_oFlash.IsActive()) {
			R_DrawFilledBox(Vector(0, 0), Vector(Wnd_GetWindowCenterX() * 2, Wnd_GetWindowCenterY() * 2), Color(255, 255, 255, 150));
		}
	}
...
class CNuke : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecNuke;
	Model m_oModel;
	SpriteHandle m_hSprNuke;
	SpriteHandle m_hSprTarget;
	SpriteHandle m_hSprTrail;
	SoundHandle m_hSndAlert;
	SoundHandle m_hSndDrop;
	Timer m_tmrWaitAlert;
	Timer m_tmrNukeDrop;
	Timer m_tmrTargetRot;
	Timer m_tmrTrail;
	bool m_bShallRelease;
	int m_iNukeSpeed;
	int m_iDecalCount;
	int m_iTrailFrame;
	float m_fTargetRot;
	...
	CNuke()
    {
		this.m_bShallRelease = false;
		this.m_iNukeSpeed = 7;
		this.m_iDecalCount = 100;
		this.m_iTrailFrame = 0;
    }
	...
	{
		this.m_vecPos = vec;
		this.m_vecNuke = Vector(vec[0] - 50, 0);
		
		this.m_hSprNuke = R_LoadSprite(g_szToolPath + "nuke.png", 1, 80, 60, 1, false);
		this.m_hSprTarget = R_LoadSprite(g_szToolPath + "target.png", 1, 64, 64, 1, false);
		this.m_hSprTrail = R_LoadSprite(g_szToolPath + "trail.png", 64, 128, 128, 8, false);
		
		this.m_hSndAlert = S_QuerySound(g_szToolPath + "alert.wav");
		this.m_hSndDrop = S_QuerySound(g_szToolPath + "drop.wav");
		S_PlaySound(this.m_hSndAlert, 9);
		
		this.m_tmrWaitAlert.SetDelay(3200);
		this.m_tmrWaitAlert.Reset();
		this.m_tmrWaitAlert.SetActive(true);
		this.m_tmrTargetRot.SetDelay(10);
		this.m_tmrTargetRot.Reset();
		this.m_tmrTargetRot.SetActive(true);
		this.m_tmrNukeDrop.SetActive(false);
		this.m_tmrTrail.SetActive(false);
	...
	void OnRelease()
	{
		...
		Ent_SpawnEntity(@expl, this.m_vecNuke);
		
		CBigDecalSprite @big = CBigDecalSprite();
		Ent_SpawnEntity(@big, Vector(this.m_vecNuke[0] - 215, this.m_vecNuke[1] - 225));
		
		for (int i = 0; i < this.m_iDecalCount; i++) {
			CDecalSprite @dcl = CDecalSprite();
			Ent_SpawnEntity(@dcl, Vector(Util_Random(1, Wnd_GetWindowCenterX() * 2 - 64), Util_Random(1, Wnd_GetWindowCenterY() * 2 - 64)));
		}
	}
	...
	void OnProcess()
	{
		if (this.m_tmrWaitAlert.IsActive()) {
			this.m_tmrWaitAlert.Update();
			if (this.m_tmrWaitAlert.IsElapsed()) {
				this.m_tmrWaitAlert.SetActive(false);
				this.m_tmrNukeDrop.SetDelay(10);
				this.m_tmrNukeDrop.Reset();
				this.m_tmrNukeDrop.SetActive(true);
				this.m_tmrTrail.SetDelay(10);
				this.m_tmrTrail.Reset();
				this.m_tmrTrail.SetActive(true);
				this.m_tmrTargetRot.SetActive(false);
				
				S_PlaySound(this.m_hSndDrop, 10);
			}
		}
		
		if (this.m_tmrTargetRot.IsActive()) {
			this.m_tmrTargetRot.Update();
			if (this.m_tmrTargetRot.IsElapsed()) {
				this.m_tmrTargetRot.Reset();
				
				this.m_fTargetRot += 0.01;
			}
		}
		
		if (this.m_tmrNukeDrop.IsActive()) {
			this.m_tmrNukeDrop.Update();
			if (this.m_tmrNukeDrop.IsElapsed()) {
				this.m_tmrNukeDrop.Reset();
				
				this.m_vecNuke[1] += this.m_iNukeSpeed;
				if (this.m_vecNuke[1] >= this.m_vecPos[1]) {
					this.m_bShallRelease = true;
					this.m_tmrNukeDrop.SetActive(false);
					this.m_tmrTrail.SetActive(false);
				}
			}
	...
	{
		if (this.m_tmrTargetRot.IsActive()) {
			R_DrawSprite(this.m_hSprTarget, Vector(this.m_vecPos[0]-43, this.m_vecPos[1]), 0, this.m_fTargetRot, Vector(-1, -1), 0.0, 0.0, true, Color(150, 0, 0, 150));
		}
		
		if (this.m_tmrTrail.IsActive()) {
			R_DrawSprite(this.m_hSprTrail, Vector(this.m_vecNuke[0] - 24, this.m_vecNuke[1] - 120), this.m_iTrailFrame, /*3.15 / 2.0*/0.0, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
		}
	
		if (this.m_tmrNukeDrop.IsActive()) {
			R_DrawSprite(this.m_hSprNuke, this.m_vecNuke, 0, 3.15, Vector(-1, -1), 0.0, 0.0, false, Color(0, 0, 0, 0));
		}
	}
...
{
	CNuke @obj = CNuke();
	Ent_SpawnEntity(@obj, Vector(vAtPos[0] + 43, vAtPos[1]));
}
