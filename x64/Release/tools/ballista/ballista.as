/*
	...
	Tool: Ballista (developed by Daniel Brendel)
	Version: 0.1
...
const int C_ARROW_SPEED = 10;
const int C_ARROW_HIT_RANGE = 50;
class CArrow : IScriptedEntity
{
	Vector m_vecPos;
	Model m_oModel;
	Timer m_tmrMovement;
	...
	//Set target position
	void SetTarget(const Vector &in vecTarget)
	{
		this.m_vecTarget = vecTarget;
	}
	...
	void Move()
	{
		...
		this.m_vecPos[0] += int(sin(this.m_fRotation + 0.014) * C_ARROW_SPEED);
		this.m_vecPos[1] -= int(cos(this.m_fRotation + 0.014) * C_ARROW_SPEED);
		...
		if (this.m_vecPos.Distance(this.m_vecTarget) < C_ARROW_HIT_RANGE) {
			this.m_bRemove = true;
		}
	}
	...
	{
		this.m_vecPos = vec;
		this.m_hSprite = R_LoadSprite(g_szToolPath + "ballista\\arrow.png", 1, 72, 72, 1, false);
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(0, 0), Vector(72, 72));
		this.m_oModel.Initialize2(bbox, this.m_hSprite);
	}
...
const int BALLISTA_TEAM_1 = 1;
const int BALLISTA_TEAM_2 = 2;
const int STOP_WALK_RANGE = 58;
class BallistaAttributes
{
	int Health;
	int Damage;
	int Speed;
...
class CBaseBallista : IScriptedEntity
{
	Vector m_vecPosition;
	Model m_oModel;
	array<SpriteHandle> m_aBody(4);
	float m_flBodyRot;
	BallistaAttributes m_sBallistaAttrs;
	Timer m_tmrAiming;
	float m_flAimDest;
	float m_flAimStep;
	Timer m_tmrMovement;
	...
	//Getters
	int GetHealth() { return this.m_sBallistaAttrs.Health; }
	int GetDamage() { return this.m_sBallistaAttrs.Damage; }
	int GetSpeed() { return this.m_sBallistaAttrs.Speed; }
	float GetRotation_() { return this.m_sBallistaAttrs.Rotation; }
	uint8 GetTeam() { return this.m_sBallistaAttrs.Team; }
	
	//Setters
	void SetHealth(int iHealth) { this.m_sBallistaAttrs.Health = iHealth; }
	void SetDamage(int iDamage) { this.m_sBallistaAttrs.Damage = iDamage; }
	void SetSpeed(int iSpeed) { this.m_sBallistaAttrs.Speed = iSpeed; }
	void SetRotation_(float fRotation) { this.m_sBallistaAttrs.Rotation = fRotation; }
	void SetTeam(uint8 ui8Team) { this.m_sBallistaAttrs.Team = ui8Team; }
	void SetBodySize(const Vector& in vec) { this.m_sBallistaAttrs.BodySize = vec; }
	void SetHeadSize(const Vector& in vec) { this.m_sBallistaAttrs.HeadSize = vec; }
	...
	void Move()
	{
		...
		//Set next position according to view
		this.m_vecPosition[0] += int(sin(this.m_flBodyRot + 0.014) * this.m_sBallistaAttrs.Speed);
		this.m_vecPosition[1] -= int(cos(this.m_flBodyRot + 0.014) * this.m_sBallistaAttrs.Speed);
	...
	void Fire()
	{
		...
		this.m_vecTargetPos = vecAbsPos;
		
		//Activate shooting timer
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		for (int i = 0; i < 4; i++) {
			this.m_aBody[i] = R_LoadSprite(g_szToolPath + "ballista\\ballista" + i + ".png", 1, 72, 72, 1, false);
		}
		this.m_hFireSound = S_QuerySound(g_szToolPath + "throw.wav");
		...
		BoundingBox bbox;
		bbox.Alloc();
		bbox.AddBBoxItem(Vector(0, 0), Vector(this.m_sBallistaAttrs.BodySize[0], this.m_sBallistaAttrs.BodySize[1]));
		this.m_oModel.Alloc();
		this.m_oModel.SetCenter(Vector(this.m_sBallistaAttrs.BodySize[0] / 2, this.m_sBallistaAttrs.BodySize[1] / 2));
	...
	void OnRelease()
	{
		...
		CExplosion@ expl = CExplosion();
		Ent_SpawnEntity(@expl, Vector(this.m_vecPosition[0] - this.m_sBallistaAttrs.BodySize[0], this.m_vecPosition[1] - this.m_sBallistaAttrs.BodySize[1]));
	...
	void OnProcess()
	{
		...
		if (this.m_tmrDirMove.IsActive()) {
			...
			this.m_flBodyRot += this.m_sBallistaAttrs.Rotation;
		...
		if (this.m_tmrFire.IsActive()) {
			...
			if (this.m_tmrFire.IsElapsed()) {
				...
				this.m_iBodyIndex++;
				if (this.m_iBodyIndex >= 4) {
					this.m_tmrFire.SetActive(false);
					this.m_iBodyIndex = 0;
					
					CArrow @arrow = CArrow();
					arrow.SetTarget(this.m_vecTargetPos);
					arrow.SetRotation(this.m_flBodyRot);
					Ent_SpawnEntity(@arrow, m_vecPosition);
				}
			}
		}
	...
	void OnDrawOnTop()
	{
		...
		if (this.GetTeam() == BALLISTA_TEAM_1) {
			sDrawingColor.r = 105;
			sDrawingColor.g = 201;
			sDrawingColor.b = 224;
			sDrawingColor.a = 255;
		...
		R_DrawSprite(this.m_aBody[this.m_iBodyIndex], this.m_vecPosition, 0, this.m_flBodyRot, Vector(-1, -1), 0.0, 0.0, true, Color(sDrawingColor.r, sDrawingColor.g, sDrawingColor.b, sDrawingColor.a));
	}
	...
	bool NeedsRemoval()
	{
		return this.m_sBallistaAttrs.Health <= 0;
	}
	...
	void OnDamage(DamageValue dv)
	{
		this.m_sBallistaAttrs.Health -= dv;
	}
	...
	DamageValue GetDamageValue()
	{
		return this.m_sBallistaAttrs.Damage;
	}
	
	//Return a name string here, e.g. the class name or instance name. This is used when DAMAGE_NOTSQUAD is defined as damage-type, but can also be useful to other entities
	string GetName()
	{
		return "CBaseBallista";
	}
...
class CBallista : CBaseBallista
{
	CBallista()
	{
		...
		this.SetTeam((g_bTeamToggle) ? BALLISTA_TEAM_1 : BALLISTA_TEAM_2);
	...
	string GetName()
	{
		return (this.GetTeam() == BALLISTA_TEAM_1) ? "Combat.Team1" : "Combat.Team2";
	}
}
...
void CDG_API_DrawOnTop()
{
	if (g_bSelectionStatus) {
		string szInfoStr = (g_bTeamToggle) ? "[Ballista] Team 1" : "[Ballista] Team 2";
