/*
	...
	Tool: Bee (developed by Daniel Brendel)
	Version: 0.1
...
class CBombDrop : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecSelSize;
	Vector m_vecTarget;
	...
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(vec[0] + 10, vec[1] + 20);
		this.m_tmrMovement.SetDelay(10);
		this.m_tmrMovement.Reset();
		this.m_tmrMovement.SetActive(true);
		this.m_hSprite = R_LoadSprite(g_szToolPath + "bomb.png", 1, 100, 100, 1, true);
		SoundHandle hSound = S_QuerySound(g_szToolPath + "drop.wav");
	...
	void OnProcess()
	{
		...
		if (this.m_tmrMovement.IsElapsed()) {
			...
			this.m_vecPos[1] += 10;
			if (this.m_vecPos[1] >= this.m_vecTarget[1]) {
				this.m_bRemoval = true;
			}
			
			this.m_fRotation += 0.1;
		}
	}
	...
	void OnDraw()
	{
		R_DrawSprite(this.m_hSprite, this.m_vecPos, 0, this.m_fRotation, Vector(50, 50), 0.0f, 0.0f, false, Color(0, 0, 0, 0));
	}
...
const int C_MAX_BOMB_DROPS = 6;
class CBee : IScriptedEntity
{
	Vector m_vecPos;
	Vector m_vecSelSize;
	Vector m_vecTarget;
	...
	CBee()
    {
		this.m_vecSelSize = Vector(0, 0);
		this.m_bRemoval = false;
		this.m_iBombCounter = 0;
    }
	
	//Called when the entity gets spawned. The position on the screen is passed as argument
	void OnSpawn(const Vector& in vec)
	{
		this.m_vecPos = Vector(-64, vec[1] - 350);
		this.m_vecTarget = vec;
		this.m_tmrMovement.SetDelay(10);
		this.m_tmrMovement.Reset();
		this.m_tmrMovement.SetActive(true);
		this.m_tmrBombing.SetDelay(125);
		this.m_tmrBombing.Reset();
		this.m_tmrBombing.SetActive(false);
		this.m_hSprite = R_LoadSprite(g_szToolPath + "bee.png", 1, 100, 100, 1, true);
		SoundHandle hSound = S_QuerySound(g_szToolPath + "bee.wav");
		S_PlaySound(hSound, 10);
		this.m_oModel.Alloc();
	}
	...
	void OnProcess()
	{
		...
		if (this.m_tmrMovement.IsElapsed()) {
			...
			this.m_vecPos[0] += 20;
			if (this.m_vecPos[0] > Wnd_GetWindowCenterX() * 2 + 64) {
				this.m_bRemoval = true;
			}
			
			if ((this.m_vecPos[0] >= this.m_vecTarget[0] - 250) && (!this.m_tmrBombing.IsActive()) && (this.m_iBombCounter < C_MAX_BOMB_DROPS)) {
				this.m_tmrBombing.SetActive(true);
			}
		}
		
		if (this.m_tmrBombing.IsActive()) {
			this.m_tmrBombing.Update();
			if (this.m_tmrBombing.IsElapsed()) {
				this.m_tmrBombing.Reset();
	
				CBombDrop @bomb = CBombDrop();
				bomb.SetTarget(this.m_vecTarget);
				Ent_SpawnEntity(@bomb, this.m_vecPos);
				
				this.m_iBombCounter++;
				if (this.m_iBombCounter >= C_MAX_BOMB_DROPS) {
					this.m_tmrBombing.SetActive(false);
				}
			}
		}
