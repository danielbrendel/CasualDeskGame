...
class CBaseTank : IScriptedEntity
{
	...
	void OnSpawn(const Vector& in vec)
	{
		...
		this.m_hSprBody = R_LoadSprite(g_szToolPath + "tc_d_body_team" + this.m_sTankAttrs.Team + ".png", 1, this.m_sTankAttrs.BodySize[0], this.m_sTankAttrs.BodySize[1], 1, false);
		this.m_hSprHead = R_LoadSprite(g_szToolPath + "tc_d_head_team" + this.m_sTankAttrs.Team + ".png", 1, this.m_sTankAttrs.HeadSize[0], this.m_sTankAttrs.HeadSize[1], 1, false);
...
void CDG_API_DrawOnTop()
{
	if (g_bSelectionStatus) {
		string szInfoStr = (g_bTeamToggle) ? "[Dual-gun] Team 1" : "[Dual-gun] Team 2";
